...
import {
  ...
  Disc, RefreshCw, Flag, Video as VideoIcon, UserCircle
} from 'lucide-react';
...
interface ContentReport {
  _id: string;
  reporter?: { firstName: string; lastName: string; email: string; profilePicture?: string };
  targetType: 'song' | 'video' | 'podcast' | 'album' | 'artist' | 'user';
  targetId?: { _id: string; title?: string; name?: string; firstName?: string; lastName?: string } | null;
  reason: string;
  description?: string;
...
const REPORT_TARGET_ICON: Record<string, React.ElementType> = {
  song: Music, video: VideoIcon, podcast: Mic, album: Disc, artist: User, user: UserCircle,
};
...
const ContentModeration: React.FC = () => {
  ...
  const [reportStatusFilter, setReportStatusFilter] = useState<'pending' | 'reviewed' | 'dismissed' | 'action_taken' | 'all'>('pending');
  ...
  const tabConfigs = [
    ...
    { label: 'Reports', type: 'reports', icon: Flag },
  ];
  const isReportsTab = tabConfigs[activeTab].type === 'reports';

  const fetchContent = async () => {
    setLoading(true);
    try {
      if (isReportsTab) {
        const response = await adminService.getContentReports(reportStatusFilter, undefined, page, 12);
        if (response.data.success && response.data.data) {
          setReports(response.data.data);
          if ((response.data as any).pagination) setTotalPages((response.data as any).pagination.pages);
        }
      } else {
  ...
  useEffect(() => {
    fetchContent();
  }, [activeTab, page, reportStatusFilter]);

  const handleResolveReport = async (reportId: string, status: 'reviewed' | 'dismissed' | 'action_taken') => {
    const loadingId = toast.loading('Updating report...');
    try {
      await adminService.resolveContentReport(reportId, status);
      toast.success('Report updated', { id: loadingId });
  ...
  const queueEmpty = isReportsTab ? reports.length === 0 : items.length === 0;
  if (loading && queueEmpty) return <Preloader isVisible={true} text="Auditing quality queue..." />;
      ...
      {isReportsTab && (
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          {(['pending', 'reviewed', 'dismissed', 'action_taken', 'all'] as const).map((s) => (
            <button
              key={s}
              onClick={() => { setReportStatusFilter(s); setPage(1); }}
              className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest whitespace-nowrap transition-all ${
                reportStatusFilter === s ? 'bg-emerald-500 text-black' : 'bg-white/5 text-zinc-500 hover:text-white'
              }`}
            >
              {s.replace('_', ' ')}
            </button>
        ...
        {isReportsTab ? (
          reports.length === 0 ? (
            ...
            >
              ...
              <p className="text-zinc-500 font-medium italic">No {reportStatusFilter !== 'all' ? reportStatusFilter.replace('_', ' ') : ''} reports.</p>
            </motion.div>
          ) : (
            <div className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {reports.map((report) => {
                  const TargetIcon = REPORT_TARGET_ICON[report.targetType] || Flag;
                  const targetLabel = report.targetId
                    ? (report.targetId.title || report.targetId.name || `${report.targetId.firstName || ''} ${report.targetId.lastName || ''}`.trim() || 'Untitled')
                    : '(deleted content)';
                  return (
                    <motion.div
                      key={report._id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                      className="premium-card group hover:border-rose-500/20 transition-all flex flex-col"
                    >
                      <div className="flex items-center gap-4 mb-6">
                        <div className="w-14 h-14 rounded-2xl bg-zinc-950 border border-white/5 overflow-hidden flex items-center justify-center flex-shrink-0">
                          <TargetIcon size={20} className="text-zinc-600" />
                        </div>
                        <div className="min-w-0">
                          <h3 className="text-sm font-bold text-zinc-900 dark:text-white truncate">{targetLabel}</h3>
                          <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest truncate">{report.targetType}</p>
                        </div>
                      </div>

                      <div className="flex-grow space-y-4 mb-8">
                        <div className="flex items-center justify-between">
                          <span className="px-2 py-0.5 bg-rose-500/10 text-rose-500 text-[9px] font-black uppercase tracking-widest border border-rose-500/20 rounded">{report.reason}</span>
                        ...
                        {report.description && (
                          <div className="bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 p-4 rounded-2xl italic">
                            <p className="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">"{report.description}"</p>
                          </div>
                        )}
                        <p className="text-[10px] text-zinc-600">
                          Reported by {report.reporter ? `${report.reporter.firstName} ${report.reporter.lastName}` : 'Unknown user'}
                        </p>
                      </div>
                      ...
                      {report.status === 'pending' ? (
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            ...
                            className="btn-secondary !py-2 !px-0 flex items-center justify-center gap-1.5 text-[10px] text-zinc-400"
                          >
                            <X size={12} /> Dismiss
                          </button>
                          <button
                            onClick={() => handleResolveReport(report._id, 'action_taken')}
                          ...
                          >
                            <ShieldAlert size={12} /> Action Taken
                          </button>
                        </div>
                      ) : (
                        <span className="text-center px-2 py-1.5 bg-white/5 text-zinc-500 text-[9px] font-black uppercase tracking-widest rounded">{report.status.replace('_', ' ')}</span>
                      )}
                    </motion.div>
                  );
                })}
